...
const questions = [
    { //Q01
        ...
        quoteAudio: "MotivationQuotes/QuoteMusic01.mp3"
    },
    { //Q02
        question: "An adult ticket to an Idaho Falls game cost $10.00 and a youth ticket cost $6.00. On Tuesday night’s game, they sold 396 tickets and grossed $3,224. How many adult tickets did they sell?  (2023 UIL Math SAC, Problem 8)",
    ...
    { //Q03
        ...
        quoteAudio: "MotivationQuotes/QuoteMusic03.mp3"
    },
    { //Q04
        question: "The <eq>5<sup>th</sup></eq> term of an arithmetic sequence is 23 and the <eq>13<sup>th</sup></eq> term is 55. Find the sum of the first 15 terms of the sequence. (nearest tenth)  (2023 UIL Math SAC, Problem 13)",
        ...
        quoteAudio: "MotivationQuotes/QuoteMusic04.mp3"
    },
    { //Q05
        question: "Amarillo Slim is playing a 5-card poker game with a well shuffled, standard 52-card deck. What is the probability that he will be dealt a full house? An example of a full house is 3 jacks and 2 queens. (nearest hundred-thousandth)  (2025 UIL Math Invitational B, Problem 39)",
        ...
        quoteAudio: "MotivationQuotes/QuoteMusic05.mp3"
    },
];
...
let backMusicVolumeZero = 0.0;
let backMusicVolumeMin = 0.05;
let backMusicVolumeMax = 0.15;
...
const quoteAudioSource = document.getElementById('quote-audio-source');
const quoteAudioToggle = document.getElementById('quote-audio-toggle');
...
backgroundMusic.volume = backMusicVolumeMax;
...
closeHint.addEventListener('click', () => {
    ...
    hintAudioToggle.textContent = 'Play Hint Music';
    backgroundMusic.volume = backMusicVolumeMax;
});
...
hintAudioToggle.addEventListener('click', () => {
    if (hintAudio.paused) {
        hintAudio.play()
        .then(() => {
            hintAudioToggle.textContent = 'Pause Hint Music';
        })
        .catch(error => console.error('Hint audio play error:', error));
        backgroundMusic.volume = backMusicVolumeZero;
    }
    else {
        hintAudio.pause();
        hintAudioToggle.textContent = 'Play Hint Music';
...
solutionVideoToggle.addEventListener('click', () => {
    ...
    backgroundMusic.volume = backMusicVolumeMin;
});
...
solutionQuoteToggle.addEventListener('click', () => {
    ...
    quoteAudio.src = q.quoteAudio;
    quoteAudio.play();
    quoteAudioToggle.textContent = 'Pause Quote Music';

    backgroundMusic.volume = backMusicVolumeZero;
});

quoteAudioToggle.addEventListener('click', () => {
    if (quoteAudio.paused) {
        quoteAudio.play()
        .then(() => {
            quoteAudioToggle.textContent = 'Pause Quote Music';
        })
        .catch(error => console.error('Quote audio play error:', error));
        ...
        quoteAudio.pause();
        quoteAudioToggle.textContent = 'Play Quote Music';
        backgroundMusic.volume = backMusicVolumeMax;
    }
});
...
function loadQuestion() {
    ...
    if (userAnswers[currentQuestion][0] === null) {  //Case-1. current question not yet tried.
        feedback.textContent = "You've got two attempts - go for it.";
        attempts = 0;
    } else {
        attempts = 1;
        if (userAnswers[currentQuestion][0] === q.correct) {  //Case-2. answer correct on 1st attempt.
            feedback.textContent = "Great Job! You crushed it on the first try - see solution above as well.";
            ...
            solutionBtn.disabled = false;
                ...
                if (userAnswers[currentQuestion][1] === null) {  //Case-3. still have 1 more attempt remaining.
                    feedback.textContent = "One shot left — check the hint and give it your best.";
                    ...
                    if (userAnswers[currentQuestion][1] === q.correct) {  //Case-4. answer correct on 2nd attempt.
                        feedback.textContent = "Sweet! You got it on the second try — see solution above as well.";
                        optionButtons[userAnswers[currentQuestion][0]].classList.add('wrong');
                        optionButtons[userAnswers[currentQuestion][1]].classList.add('correct');
                        optionButtons.forEach(btn => btn.disabled = true);
                        solutionBtn.disabled = false;
                    } else {  //Case-5. answer wrong on both attempts.
                        feedback.textContent = `You've used both attempts on this one - See solution above.`
...
function checkAnswer(choice) {
    ...
    if (choice === q.correct) {
        ...
        feedback.textContent = 'Awesome! You got it. Also, check out the solution above.';
        feedback.style.background = 'rgb(144, 238, 144)';
        optionButtons.forEach(btn => btn.disabled = true);
        hintBtn.disabled = true;
        solutionBtn.disabled = false;
        score ++;
        ...
        feedback.textContent = 'Nope. See the hint and try again!';
        feedback.style.background = 'rgb(255, 153, 153)';
        hintBtn.disabled = false;
        if (attempts === 2) {
            feedback.textContent = `Maximum attempts reached (2). The answer is: ${q.options[q.correct]}. Check out the solution above.`;
            optionButtons[q.correct].classList.add('correct');
            optionButtons.forEach(btn => btn.disabled = true);
            hintBtn.disabled = true;
            solutionBtn.disabled = false;
    ...
    if (attempts === 1) { numQuestionsAttempted ++; }
    if (numQuestionsAttempted === questions.length) { submitBtn.disabled = false; }
}
